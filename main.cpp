...
void keyboard(GLFWwindow* window, Quad* quad_array);
...
int main(int argc, const char *argv[]) {
    ...
    Quad quads[2] = {
        Quad(vss, fss, vec3(0.5,0.5,-10.0), 1.0, 0.0, 0.0),
        Quad(vss, fss, vec3(0.5,0.5,1.0), 0.0, 1.0, 0.0),
    };
                
    while (!glfwWindowShouldClose(window)) {
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        
        keyboard(window, quads);
...
void keyboard(GLFWwindow* window, Quad quad_array[]) {
    ...
    mat4 projection = perspective(radians(90.f), 1.f, .01f, 2000.f);
    mat4 view = lookAt(camera_position, camera_position+camera_eye, camera_up);
